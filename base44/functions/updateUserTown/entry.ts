...
Deno.serve(async (req) => {
  try {
    ...
    if (user.role === 'superadmin' && body.acting_town_id && town_id && town_id !== body.acting_town_id.trim()) {
      return Response.json(
        { error: 'Forbidden: while impersonating, new town must be the active municipality' },
        { status: 403 }
      );
    }
