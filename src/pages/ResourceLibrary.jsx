...
export default function ResourceLibrary() {
  ...
  // Aligned with your Base44 Security Rules
  ...
  async function loadResources() {
    setLoading(true);
    try {
      // Filtering by town_id to satisfy the Entity-User Field Comparison rule
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <div className="relative w-48 sm:w-64">
              ...
              <Input placeholder="Filter..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-9 h-8 text-xs bg-white border-slate-200" />
            </div>
...
function AICuratePanel({ onClose, townId, townName, state }) {
  ...
  async function sendMessage(e, customMsg = null) {
    ...
    const prompt = `
      ...
      This is required to pass the Base44 security rules.
  ...
  const handleAutoCurate = () => {
    sendMessage(null, `Build out the library for ${townName}, ${state} with robust land-use resources.`);
  };
  ...
  return (
    <div className="mb-8 border border-slate-200 rounded-xl overflow-hidden bg-white shadow-xl">
      <div className="flex items-center justify-between px-4 py-3 bg-slate-900 text-white font-mono text-[10px]">
        <span className="font-bold uppercase tracking-widest">Registrar Curator</span>
