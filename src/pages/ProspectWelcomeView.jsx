...
export const SUPPORT_EMAIL = 'support@code-enforce.com';
...
/**
 * @param {object} props
 * @param {string | null} props.linkBase - If set (e.g. https://www.code-enforcepro.com), public tools + dashboard use absolute URLs. If null, use in-app <Link> paths.
 * @param {() => void} props.onSignIn
 * @param {() => void} props.onSubscribeNewTown
 * @param {boolean} [props.showOpenApp]
 * @param {() => void} [props.onOpenApp]
 * @param {boolean} [props.marketingSiteFooter] - copy for code-enforce.com-style hosting
 */
export default function ProspectWelcomeView({
  linkBase,
  onSignIn,
  onSubscribeNewTown,
  showOpenApp = false,
  ...
  const reportHref = linkBase ? `${linkBase}/report` : '/report';
  const portalHref = linkBase ? `${linkBase}/public-portal` : '/public-portal';
          ...
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              {showOpenApp && onOpenApp && (
                <Button variant="ghost" size="sm" className="hidden text-slate-600 sm:inline-flex" type="button" onClick={onOpenApp}>
                  Open app
                </Button>
              )}
              ...
              <Button size="sm" className="bg-blue-600 text-white shadow-md shadow-blue-600/20 hover:bg-blue-700" type="button" onClick={onSignIn}>
                Sign in
              </Button>
            </div>
          </div>
                  ...
                  {linkBase ? (
                    <>
                      <Button variant="secondary" size="sm" className="text-xs" asChild>
                        <a href={reportHref}>Report a concern</a>
                      </Button>
                      <Button variant="outline" size="sm" className="text-xs" asChild>
                        <a href={portalHref}>Case lookup</a>
                      </Button>
                    </>
                  ) : (
                    <>
        ...
        <footer className="border-t border-slate-200 bg-white px-4 py-10 sm:px-6">
          <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 text-center sm:flex-row sm:text-left">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 text-sm text-slate-600 sm:justify-end">
              ...
              <button type="button" className="font-medium text-blue-600 hover:underline" onClick={onSignIn}>
                Staff sign in
              </button>
              <button type="button" className="hover:text-slate-900" onClick={onSubscribeNewTown}>
                Subscribe
              </button>
            </div>
          </div>
          {marketingSiteFooter && linkBase ? (
            <p className="mx-auto mt-8 max-w-6xl text-center text-xs text-slate-500 sm:text-left">
              This site is for information only. The application — sign-in, subscriptions, and case data — is hosted at{' '}
              <a href={linkBase} className="font-medium text-blue-600 hover:underline">
                {linkBase.replace(/^https?:\/\//, '')}
              </a>
              .
            </p>
          ) : (
